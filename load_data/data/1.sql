...
poli_id, poli_name, doctor_id, doctor_name, tags, penunjang_tags, hospital_id,rate, rate_a,rate_b, rate_c, rate_d, rate_e, 
rate_f, rate_g, rate_h, rate_i, rate_j, procedure_type,
...
meta_data, table_ref, column_ref, ref_id, sync_ref_id, ref_code, procedure_id_ris, inhealth_id, is_active, created_date,
created_by, created_by_name, modified_date, modified_by, modified_by_name)


WITH union_procedure AS (
  SELECT 
...
SELECT 
    ...
    stg_1.procedure_key, 
    NULL AS description, 
    mjp.jns_pelayanan_id, 
    mjp.jns_pelayanan_name, 
    mm.model_id AS for_pelayanan_id, 
    stg_1.for_pelayanan_name,
    NULL AS grp_id, 
    sub_rr.grp_ids, 
    NULL AS poli_id, 
    NULL AS poli_name, 
...
FROM stg_1
LEFT JOIN (
	SELECT stg_1.procedure_id, 
	GROUP_CONCAT(DISTINCT CONCAT(kr.rr_code, kr.kelas_rr_id) SEPARATOR ',') AS grp_ids
	FROM stg_1
